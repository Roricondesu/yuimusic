 ...
 * `<Glass>` primitive and its opt-in motion utilities. Copied from the
 * official liquid-glass examples and restyled for the music app.
 */
...
export const GlassSlider: React.FC<GlassSliderProps> = ({
  ...
  const refractionTrackH = refractionTrackHeight ?? trackH;
  ...
  const mv = useMemo(() => {
    ...
    const trackScaleX = glassValue(1);
    const trackScaleY = glassValue(1);
  ...
  const track = trackColor ?? (isDark ? "rgba(255,255,255,0.22)" : "rgba(0,0,0,0.16)");
